...
**Desde 2026-08-03, sistema em produção** — `https://sandrofreiresf.online`, VPS Hostinger **compartilhada com VillaMill e Sistema Thieco** (mesmo IP `2.24.93.178` dos dois). **Desde 2026-08-12 (confirmado, não datado com precisão — só descoberto nessa sessão), tem repositório próprio e privado no GitHub** (`github.com/willianslegacy94-zion/academia-sandro.git`) — **não é mais** subpasta do monorepo `zion-workspace` (ver [[arquitetura-academiasandro]], seção 8, pra detalhe da migração e o que isso muda de governança). Banco é Supabase (nuvem), **mesmo banco em dev e produção** — não existe separação, cuidado ao rodar script/seed local, ele mexe no banco real.
...
| Repositório         | `github.com/willianslegacy94-zion/academia-sandro.git` — **privado**, próprio deste projeto (2026-08-12). `git push origin main` direto funciona, sem intermediação de agente — diferente da regra antiga de monorepo (ver [[arquitetura-academiasandro]] seção 8) |
...
**Não existe `git pull` neste projeto — nem clone git na VPS (decisão confirmada, não limitação técnica, 2026-08-12).** `/opt/academia-sandro` é só uma cópia de arquivos sincronizada via `rsync`, sem `.git` próprio. O `git push` pro GitHub é só backup/histórico do código — não alimenta o deploy sozinho, precisa sempre do `rsync` acima. **Isso já foi reavaliado explicitamente em 2026-08-12** — agora que o projeto tem repo próprio e privado (ver tabela acima), adotar `git clone`/`git pull` na VPS ficaria mais simples (deploy de 1 comando em vez de rsync + lembrar de excluir `node_modules`/`.next`/`.git`). O caminho pra isso (não implementado, só desenhado) seria uma **deploy key SSH** (par de chaves gerado na VPS, cadastrado como "Deploy key" read-only no GitHub, `~/.ssh/config` com um `Host` alias apontando `IdentityFile` pra ela) e depois `git init && git remote add origin <ssh-url-com-alias> && git fetch && git reset --hard origin/main` na pasta já existente (`reset --hard`, não `checkout`, porque a pasta já tem todos os arquivos soltos da era rsync — `checkout` recusaria por causa deles, `reset --hard` sobrescreve sem reclamar e não toca em nada fora da árvore versionada, tipo `.env.production`). **Perguntado diretamente, o usuário optou por continuar com rsync por ora** (Opção A) — não é uma tarefa pendente, é a forma de deploy atual por escolha.

**Achado nesse mesmo dia (2026-08-12): o `rsync` documentado acima copia o `.env` de dev pro VPS junto, sem querer — mas é inofensivo.** O comando de rsync não exclui `.env` (só `node_modules`/`.next`/`.git`), então o `.env` local (com a `CRON_SECRET` de teste, connection string de dev, etc.) aparece solto em `/opt/academia-sandro/.env` depois do sync. **Não é um risco real:** `.dockerignore` exclui `.env*` (exceto `.env.production.example`) do contexto de build da imagem Docker, e todo serviço do `docker-compose.yml` (`app`/`migrate`/`seed`) tem `env_file: .env.production` explícito — o `.env` solto nunca é lido em lugar nenhum do pipeline. Pode ser apagado por organização (`rm /opt/academia-sandro/.env` no VPS) mas não precisa, não afeta nada.
...
### WhatsApp real via Evolution API (2026-08-03, muito expandido em 2026-08-12)

**Desde 2026-08-12, a maioria dos avisos do sistema já é automática** — `src/lib/whatsapp.ts` continua existindo só pra montar o *texto* das mensagens (`mensagemCobranca`, `mensagemAcessoPortal`, `mensagemAniversario`, `mensagemConfirmacaoAula`) e o link `wa.me` de clique manual usado como fallback em algumas telas (ex: reenviar acesso manualmente na ficha do aluno) — quem manda de verdade, sem clique humano, é sempre `src/lib/whatsapp-gateway.ts#enviarWhatsapp`. Lista completa dos avisos automáticos hoje:
- **Bloqueio de agenda avisa o aluno** (2026-08-03) — `criarBloqueio` (Configurações → Agenda) resolve quem tem acesso a algum horário dentro da janela bloqueada e manda mensagem pra cada um
- **Aula experimental agendada avisa o admin** (2026-08-03) — `criarPreCadastro` (`/matricule-se`), se a pessoa marcou uma data, avisa o telefone do `Usuario` com `username = ADMIN_USERNAME`, agora **com 2 links de confirmar/recusar** (2026-08-12, ver abaixo)
- **Acesso ao portal (usuário + link de definir senha) avisa o próprio aluno** (2026-08-12) — `enviarAcessoPortalWhatsapp` (`src/lib/acesso-portal.ts`), chamada pelos 2 fluxos de **cadastro** (autocadastro `/cadastro-aluno` e admin criando aluno novo/aprovando pré-cadastro em `createAluno`). **Não** dispara na criação manual de acesso pra aluno já existente (`criarAcessoAluno`) — essa continua só com o botão/link manual na tela, por decisão do usuário.
- **Novo autocadastro avisa o admin** (2026-08-12) — mesmo padrão do aviso de aula experimental, dentro de `criarAlunoAutocadastro` (`/cadastro-aluno`)
- **Cobrança de vencendo/atrasado, via cron diário** (2026-08-12) — ver seção "Cron de cobrança e aniversário" acima
- **Parabéns de aniversário, via cron diário** (2026-08-12) — idem
- **Confirmação de aula experimental avisa o lead** (2026-08-12) — quando o admin clica no link de confirmar/recusar recebido no aviso de aula experimental, `api/aula-experimental/[id]/confirmar` grava a decisão e avisa automaticamente o telefone do lead com o resultado. Decisão de design: **não** usar botões nativos do WhatsApp (instáveis nesse tipo de gateway não-oficial) — são 2 links de clique normal.
...
### Cron de cobrança e aniversário automáticos por WhatsApp (2026-08-12)

Diferente do cron acima, essas duas rotas **disparam mensagem de verdade** pros alunos — por isso exigem `CRON_SECRET` na query string, senão respondem `401` sem tocar no banco. Gerar/confirmar o secret antes:
```bash
ssh root@2.24.93.178
cd /opt/academia-sandro
grep CRON_SECRET .env.production || echo 'CRON_SECRET="'$(openssl rand -base64 32)'"' >> .env.production
grep CRON_SECRET .env.production   # copia o valor pras 2 linhas de crontab abaixo
```
Depois, `docker compose up -d --force-recreate app` se o `.env.production` mudou (senão o container continua com o valor antigo em memória). No crontab (`crontab -e`):
```
0 9 * * * curl -s "https://sandrofreiresf.online/api/cron/cobranca?secret=SEU_CRON_SECRET" > /dev/null
0 8 * * * curl -s "https://sandrofreiresf.online/api/cron/aniversario?secret=SEU_CRON_SECRET" > /dev/null
```
`cobranca` cobre quem está vencendo (0-3 dias) ou atrasado (sem limite), no máximo 1 aviso por dia por aluno (`Aluno.ultimoAvisoCobrancaEm`) — se `enviarWhatsapp` falhar, o campo não é atualizado e o cron do dia seguinte tenta de novo, não fica "travado" achando que já avisou. `aniversario` cobre quem faz aniversário no dia (compara mês/dia de `dataNascimento`), mesmo padrão de controle de duplicidade (`Aluno.ultimoParabensEm`). **Pendente confirmar nesta sessão (2026-08-12):** as duas linhas de crontab foram passadas pro usuário rodar, mas não houve confirmação de volta (`crontab -l`) de que foram de fato adicionadas — checar numa sessão futura antes de assumir que estão ativas.

**Testando sem esperar o horário do cron:** as duas rotas aceitam chamada manual a qualquer momento (`curl "https://sandrofreiresf.online/api/cron/cobranca?secret=..."`) — útil pra validar que o `CRON_SECRET` está certo e ver o resultado (`{"enviados": N, "falhas": N}`) sem esperar `0 9 * * *`. Rodar de novo no mesmo dia não duplica envio pra quem já foi avisado (ver controle de duplicidade acima), só reprocessa quem falhou.
...
- **Sem navegador real disponível neste ambiente (achado 2026-08-12) — Chromium baixa mas não roda por falta de libs do sistema, sem `sudo` pra instalar.** `chromium-cli` não está instalado; `playwright-core` + o Chromium já cacheado (`~/.cache/ms-playwright/`) rodam, mas o binário falha na hora de abrir (`error while loading shared libraries: libnspr4.so`) — falta `libnss3`/`libnspr4`/etc., e `sudo apt-get install` pede senha interativa (sem acesso). **Não vale insistir tentando contornar isso a cada sessão** — assumir que não vai ter browser headless funcional aqui, a não ser que o usuário resolva as libs do lado dele. **Alternativa que funciona bem pra validar Server Actions sem UI:** escrever um script `.ts` temporário em `scripts/_tmp-*.ts` (fora do controle de versão — apagar no final), importando `@/lib/prisma` e as funções de lib relevantes (ex: `criarUsuarioAluno`, `enviarAcessoPortalWhatsapp`) diretamente, montando os dados de teste manualmente e chamando a lógica de negócio "pelada" — funciona porque a maior parte da lógica fica em módulos `src/lib/*.ts` sem `"use server"`, só as `actions.ts` (que dependem de `headers()`/`redirect()`, exclusivos de dentro de uma request Next.js real) não dão pra chamar direto assim. Rodar com `npx tsx scripts/_tmp-nome.ts` (precisa de `import "dotenv/config"` no topo pra carregar o `.env`). **Sempre limpar os dados de teste no final** (`prisma.aluno.deleteMany({ where: { nome: "Teste Claude ..." } })` ou equivalente) — é o banco de produção, não um banco de teste isolado. Pra testar uma rota HTTP específica (ex: `api/cron/*`), dá pra combinar com `curl` contra o `npm run dev` local depois de confirmar a lógica de banco separadamente.
...
**Recorreu de novo em 2026-08-12, mesmo padrão:** ao longo de uma sessão longa com vários restarts de `npm run dev` (rodado em background, `&`), chegaram a acumular **3 processos `next dev` simultâneos** escutando/competindo pela mesma porta, todos escrevendo no mesmo `.next/dev/cache/turbopack/` — sintoma foi erro genérico `Compaction failed: Another write batch or compaction is already active` nos logs, e rotas novas devolvendo 404/500 mesmo com o código certo no disco. `lsof -ti:3000 -sTCP:LISTEN | xargs kill` sozinho **não é suficiente** nesse ambiente — várias vezes deixou processo(s) vivo(s) pra trás (provavelmente por timing: o novo `npm run dev` já tinha subido antes do kill terminar de liberar a porta, ou processos zumbis de sessões anteriores nem apareciam no `lsof` daquele momento). **Checagem que resolve de vez:** `ps aux | grep -i "next dev\|next-server" | grep -v grep` — lista todo PID de verdade, mata cada um explicitamente (`kill -9 <pids>`), só *depois* confirma porta livre e sobe um único `npm run dev`. Fazer isso como primeiro passo sempre que o servidor de dev parecer "não pegar" uma mudança, antes de desconfiar de cache do Next ou do código.
- **`git push` NÃO é mais restrito a um agente específico neste projeto (atualizado 2026-08-12)** — a regra antiga de "só o `aiox-devops` publica" existia por causa do risco do monorepo compartilhado `zion-workspace` (árvore suja, projetos vizinhos, risco de arrastar arquivo de outro sistema). Desde que `academia-sandro` ganhou repositório próprio e privado (ver tabela no topo deste documento e [[arquitetura-academiasandro]] seção 8), esse risco específico não existe mais aqui — `git add`/`git commit`/`git push origin main` foram feitos diretamente nesta sessão (2026-08-12), várias vezes, sem intermediação, sem incidente. **Não confirmado:** se outros projetos do workspace (sistema-thieco, orbita-lobo, etc.) passaram pela mesma migração — não assumir que a regra caiu pra eles também sem checar o `git remote -v` de cada um primeiro.
