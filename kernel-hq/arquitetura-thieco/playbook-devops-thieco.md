...
- **Contrato do Cortex mudou** (`7c80eff`, efeito colateral da portabilidade pro whitelabel — ver [[playbook-devops-orbita-whitelabel]]): `notificarAdminViaCortex` (`backend/routes/notificacoes.js:46`) agora manda `instancia: 'thieco-admin'` direto no payload pro endpoint `POST {CORTEX_URL}/api/v1/cortex/notificar-admin`, em vez do `tenant_id` resolvido contra um dicionário fixo dentro do Cortex. Mesmo destino real, comportamento idêntico pro thieco — só muda se algum dia o Cortex mudar de novo o contrato desse endpoint (aí é aqui que se ajusta, não em nenhum outro lugar).
